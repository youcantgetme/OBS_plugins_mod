...
#define TEXT_METHOD_SCREEN  obs_module_text("Capture via screen capture method. Caution! other window could cover it")
...
#define T_BKBAR_ENABLE		obs_module_text("Enable extended title bar*")
#define T_COLOR             obs_module_text("Title text color")
#define T_BKCOLOR           obs_module_text("Background color")
#define T_TITLE_SHADOW      obs_module_text("Title text shadow")
/* clang-format on */
...
struct window_capture {
	...
	bool crop_shadow;
	bool enable_title_bar;
	bool title_shadow;
	uint32_t bk_color;
	uint32_t color;
	wchar_t titletext[500];
...
{
	return (rgb & 0xFF) | (rgb & 0xFF00) | (rgb & 0xFF0000);
}
static void update_settings(struct window_capture *wc, obs_data_t *s)
{
	...
	uint32_t new_bk_color = obs_data_get_uint32(s, "bk_color");
	wc->bk_color = rgb_to_bgr(new_bk_color);
	uint32_t new_color = obs_data_get_uint32(s, "color");
	wc->color = rgb_to_bgr(new_color);
	wc->title_shadow = obs_data_get_bool(s, "title_shadow");
	wc->enable_title_bar = obs_data_get_bool(s, "enable_title_bar");
}
...
static void wc_defaults(obs_data_t *defaults)
{
	...
	obs_data_set_default_int(defaults, "color", 0x101010);
	obs_data_set_default_int(defaults, "bk_color", 0xf3f3f3);
	obs_data_set_default_bool(defaults, "enable_title_bar", false);
	obs_data_set_default_bool(defaults, "title_shadow", true);
}
				       ...
				       struct window_capture *wc)
{
	...
	const bool screen_options = method == METHOD_SCREEN;
...
static bool EnableTitleBar(obs_properties_t *props, obs_property_t *p,
				  obs_data_t *settings)
{
	bool enable_title_bar = obs_data_get_bool(settings, "enable_title_bar");
	p = obs_properties_get(props, "bk_color");
	obs_property_set_visible(p, enable_title_bar);
	p = obs_properties_get(props, "color");
	obs_property_set_visible(p, enable_title_bar);
	p = obs_properties_get(props, "title_shadow");
	obs_property_set_visible(p, enable_title_bar);
...
static obs_properties_t *wc_properties(void *data)
{
	...
	obs_property_list_add_int(p, TEXT_METHOD_SCREEN, METHOD_SCREEN);
	...
	p = obs_properties_add_bool(ppts, "enable_title_bar", T_BKBAR_ENABLE);
	obs_property_set_modified_callback(p, EnableTitleBar);
	obs_properties_add_color(ppts, "color", T_COLOR);
	obs_properties_add_color(ppts, "bk_color", T_BKCOLOR);
	p = obs_properties_add_bool(ppts, "title_shadow", T_TITLE_SHADOW);
...
static void wc_tick(void *data, float seconds)
{
	...
	if (wc->method == METHOD_BITBLT || wc->method == METHOD_SCREEN) {
		...
		if (reset_capture) {
			...
			dc_capture_init(&wc->capture, 0, 0,
					...
					wc->compatibility);
			GetWindowTextW(wc->window, wc->titletext, 499);
		}
		wc->capture.color = wc->color;
		wc->capture.bk_color = wc->bk_color;
		wc->capture.enable_title_bar = wc->enable_title_bar;
		wc->capture.title_shadow = wc->title_shadow;
		wc->capture.titletext = wc->titletext;
		if (wc->method == METHOD_SCREEN) {
			wc->capture.x = rect.left;
			wc->capture.y = rect.top;

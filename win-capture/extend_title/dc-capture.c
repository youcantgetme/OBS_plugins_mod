...
static void draw_cursor(struct dc_capture *capture, HDC hdc, HWND window)
{
	...
	if (GetIconInfo(icon, &ii)) {
		...
		DrawIconEx(hdc, pos.x, pos.y + capture->title_pad, icon, 0, 0,
			   0, NULL, DI_NORMAL);
...
void dc_capture_capture(struct dc_capture *capture, HWND window)
{
	HDC hdc_target;
	HDC hdc;
	if (capture->enable_title_bar)
		capture->title_pad = 18;
	else
		capture->title_pad = 0;
	...
	BitBlt(hdc, 0, capture->title_pad, capture->width, capture->height,
	       hdc_target,
	...
	if (capture->enable_title_bar) {
		...
		rect.left = 0;
		rect.right = capture->width;
		rect.top = 0;
		rect.bottom = capture->title_pad;
		HBRUSH brush = CreateSolidBrush(capture->bk_color);
		FillRect(hdc, &rect, brush);
		DeleteObject(brush);

		SetBkColor(hdc, capture->bk_color);
		rect.left = 2;
		rect.top = 2;
		rect.right = capture->width - 2;
		rect.bottom = capture->height - 2;
		SetBkMode(hdc, TRANSPARENT);
		if (capture->title_shadow) {
			SetTextColor(hdc, RGB(10, 10, 10));
			DrawText(hdc, capture->titletext, -1, &rect,
				 DT_SINGLELINE);
		}
		rect.left -= 1;
		rect.top -= 1;
		rect.right -= 1;
		rect.bottom -= 1;
		SetTextColor(hdc, capture->color);
		DrawText(hdc, capture->titletext, -1, &rect,
			 DT_SINGLELINE);
	}

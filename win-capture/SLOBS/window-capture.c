...
#define TEXT_CROP_SHADOW    obs_module_text("Enable border shadow crop")
...
static void update_settings(struct window_capture *wc, obs_data_t *s)
{
	...
	wc->screen_capture = obs_data_get_bool(s, "screen_capture");
...
static void wc_defaults(obs_data_t *defaults)
{
	...
	obs_data_set_default_bool(defaults, "screen_capture", false);
	obs_data_set_default_bool(defaults, "crop_shadow", true);
}

static bool EnableScreenCapture(obs_properties_t *props, obs_property_t *p,
			       obs_data_t *settings)
{
	bool screen_capture = obs_data_get_bool(settings, "screen_capture");
	p = obs_properties_get(props, "crop_shadow");
	obs_property_set_visible(p, screen_capture);
}
static obs_properties_t *wc_properties(void *unused)
{
	...
	p = obs_properties_add_bool(ppts, "screen_capture", TEXT_METHOD_SCREEN);
	obs_property_set_modified_callback(p, EnableScreenCapture);
...
static void wc_tick(void *data, float seconds)
{
	...
	if (reset_capture) {
		...
		dc_capture_free(&wc->capture);
		if (wc->screen_capture)
		dc_capture_init(&wc->capture, 0, 0, rect.right - rect.left,
			rect.bottom - rect.top, wc->cursor, wc->compatibility);
			else
	...
	if (wc->screen_capture) {
		...
		wc->capture.width = rect.right - rect.left;
		wc->capture.height = rect.bottom - rect.top;
